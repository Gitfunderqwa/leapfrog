...
class App extends React.Component {
  ...
  buildpdf(term) {
    ...
    if (term.propertyTitle === '') {

      alert("Please fill in all the fields")

    } else {

    let SBAvg = (parseInt(term.SbMaxVal) + parseInt(term.SbMinVal)) / 2
    SBAvg = numberWithSpaces(SBAvg)
    ...
    doc.setFontSize(24)
    doc.text(`R${numberWithSpaces((term.squareMeter*term.avgPriceSquareMeter).toFixed(0))}`, 20, 90)
    doc.setFontSize(10)
...
Average growth in the area between 2015 and 2020 was calculated at ${term.rate}%
Which is a 2.75% average growth per year.`, 20, 140)
    ...
    doc.setFontSize(24)
    doc.text(`R${numberWithSpaces(term.historyPuchase)}`, 20, 185)
...
by a Professional Property Valuator. `, 20, 60)
    ...
    doc.text(`-R${numberWithSpaces((term.squareMeter*term.avgPriceSquareMeter).toFixed(0))}`, 125, 88)
    doc.text(`Standard Bank Desktop evaluation`, 20, 96)
    doc.text(`-R${SBAvg}`, 125, 96)
    doc.text(`FNB Desktop Evaluation`, 20, 104)
    doc.text(`-R${numberWithSpaces(term.FNBVal)}`, 125, 104)
    doc.text(`Time Value evaluation`, 20, 112)
    doc.text(`-R${SBAvg}`, 125, 112)
    doc.text(`Average listing price for similar units`, 20, 120)
    doc.text(`-R${SBAvg}`, 125, 120)
